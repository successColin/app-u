	...
	<view class="mpvue-picker">
		...
		<view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}" >
			<view class="mpvue-picker__hd" catchtouchmove="true">
				<view class="mpvue-picker__action" @click="pickerCancel">{{ $i18nMsg('basic-cancel') }}</view>
				<view class="mpvue-picker__action" :style="{color:themeColor}" @click="pickerConfirm">{{ $i18nMsg('basic-confirm') }}</view>
			</view>
			<!-- 单列 -->
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange"
			 v-if="mode==='selector' && pickerValueSingleArray.length > 0 && showPicker">
				<block>
					<picker-view-column>
			...
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange"
			 v-if="mode==='datetimeSelector'">
				<block>
					...
					<picker-view-column>
						<view class="picker-item" v-for="(item,index) in pickerValueSecond" :key="index">{{item.label}}</view>
					</picker-view-column>
				</block>
			</picker-view>
			<!-- 时间选择器 -->
		...
		methods: {
			...
			initPicker(valueArray) {
				...
				try {
					...
					if (this.mode === 'selector') { // 单列选择器
					...
					} else if (this.mode === 'dateSelector' || this.mode === 'datetimeSelector') { // 日期选择器
						let yearList = [];
						let monthList = [];
						let dayList = [];
						...
						if (this.mode === 'datetimeSelector') { // 日期时间选择器
							let hourArray = [];
							let minuteArray = [];
							let secondArray = [];
							for (let i = 0; i < 24; i++) {
			...
			pickerCancel() {
				...
				this.pickerValueArrayChange = true;
				this.modeChange = true;
				const valueObj =  this._getPickerLabelAndValue(this.pickerValue, this.mode)
				let pickObj = {
			...
			_getPickerLabelAndValue(value, mode) {
				...
				if (mode === 'selector') {
					...
					pickerLable =
						`${this.pickerValueYear[value[0]].label}-${this.pickerValueMonth[value[1]].label}-${this.pickerValueDay[value[2]] ? this.pickerValueDay[value[2]].label : this.pickerValueDay[this.pickerValueDay.length - 1].label}`;
					pickerGetValue.push(this.pickerValueYear[value[0]].value);
					pickerGetValue.push(this.pickerValueMonth[value[1]].value);
					pickerGetValue.push(this.pickerValueDay[value[2]] ? this.pickerValueDay[value[2]].value : this.pickerValueDay[this.pickerValueDay.length - 1].value);
					if (mode === 'datetimeSelector') {
						pickerLable +=
							` ${this.pickerValueHour[value[3]].label}:${this.pickerValueMinute[value[4]].label}:${this.pickerValueSecond[value[5]].label}`;
			...
			_initPickerValue() {
				...
				if (this.pickerValueDefault.length === 0) {
					if (this.mode === 'selector') {
						tempPickerValue = [0];
					} else if (this.mode === 'dateSelector' || this.mode === 'datetimeSelector') {
					...
					if (this.mode === 'dateSelector' || this.mode === 'datetimeSelector') {
						tempPickerValue = getDatePickerIndex(this.pickerValueDefault,this.range[0]);
					} else {
