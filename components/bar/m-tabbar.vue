<!-- 底部导航栏 -->
<template>
	<view class="m-bar" :class="[`${position}_Bar`,themeClass_]" :style="styleObject_">
		<view v-for="(item,index) in barList" class="m-bar-item" 
			:class="{active:index+1===activityIndex,'m-bar-big':item.isbig}" 
			:key="index" 
			:ref="item.barKey"
			@click="handleClick(index+1)">
			<view :class="{animated:index+1===activityIndex,tada:index+1===activityIndex}" v-if="!item.isbig">
				<m-font-icon :icon="item.bundleIconName || item.icon" :iconSize="item.isbig ? bigSize : iconSize" ></m-font-icon>
			</view>
			<m-font-icon v-else :icon="item.bundleIconName || item.icon" :iconSize="item.isbig ? bigSize : iconSize" ></m-font-icon>
...
<script>
	...
	export default {
		...
		computed: {
			...
			activityBar(){
				if(this.activityIndex !== 0 && this.barList.length) return this.barList[this.activityIndex-1]
				else return {}
			}
		},
		watch:{
		...
		methods: {
			...
			tabActivityBar(showpage,barId){//选中tab时做的操作
				if(!showpage){
					const _this = this
					uni.showModal({
					...
					if(showpage in CHART_PATH) { //特制化图表
						uni.navigateTo({
							url:CHART_PATH[showpage]
						})
					}else if( /^https:\/\/.*/i.test(showpage) || /^http:\/\/.*/i.test(showpage) ){//如果是http接口
						let key = `bar_${barId}`
						this.SET_COMMON_WEBVIEW({
							key:key,
							url:showpage
						})
						uni.navigateTo({//调整webview界面
