...
<template>
	...
	<!-- #endif -->
		...
		<!-- #endif --> 
		<!-- #ifdef MP -->  
		<view class="backbar_title" v-if="backStyle !== 'backStyleNoBack'"><view class="content">{{ appName }}</view></view> 
		<!-- #endif -->
		<view class="backbar" :class="[backStyle]" :style="{background: backStyle === 'backStyleNoBack' ? themeColorCode:''}">
			...
			<view class="backBtn bar-left">
				<m-button v-if="backStyle !== 'backStyleNoBack'" :isFontBtn="true" @elementClick="hangdleBackClick">
					<m-font-icon icon="iconfanhui" :iconSize="40" slot="buttonIcon" :color="colorCode"></m-font-icon>
				</m-button>
...
<script>
	...
	export default {
		...
		computed: {
			...
			backStyleObj(){
				return {
					statusBarBgColor: this.statusBarBgColor !== "transparent" ? this.themeColorCode : 'transparent' 
				}
			}
		},
		methods: {
