	...
	<view v-if="element.visiabled">
		<view class="formElemets" 
		 ...
		 element.readonly?'readonly':'']">
			<label class="text" :class="selectLabel || element.readonly?'':'drakcolor'">
				{{ element.label }}
				<span v-if="element.requisite" class="icon iconfont iconmust-fill2 requisite"></span>
			</label>
			<view class="content" :class="{hasrelationTab:element.relationTabId}" @tap="showPicker">
				<input class="input" :value="selectLabel" :placeholder="element.placeholderText" disabled="true"/>
				<m-button class="normalbutton" :isFontBtn="true">
					<block slot="buttonIcon" style="display: inline-block;">
						<m-normal-icon icon="APP_31" :iconSize="pxToREM(36)" color="rgba(0, 0, 0, 0.25)"></m-normal-icon>
					</block>
				</m-button>
			</view>
		</view>
	...
	export default {
		...
		computed: {
			...
			elementOption() {
				...
				dropDownValue.unshift({value:'',label:this.$i18nMsg('formTip-select')})
				return dropDownValue.find(item => item.value == this.value) || {value:"",lable:this.$i18nMsg("formTip-select")} // 中文：请选择数据
			},
			selectLabel() {
