...
<!-- 	图片上传控件 -->
	<view class="formElemets form-upimg" v-if="element.visiabled" :class="[element.readonly?'readonly':'']">
		<label class="text" :class="upload_picture_list.length>0?'':'drakcolor'">
			{{ element.label }}
		...
		<scroll-view scroll-x>
			...
			<view v-for="(item,index) in upload_picture_list" :key="index" class="sunsin_picture_item">
				...
				<text class='del iconfont icon-shanchu' :class="upImgConfig.delBtnLocation=='left'?'left':upImgConfig.delBtnLocation=='right'?'right':upImgConfig.delBtnLocation=='bleft'?'bleft':upImgConfig.delBtnLocation=='bright'?'bright':'right'"
				 ...
				 :hidden="upImgConfig.isDelIcon || false" v-if="!element.readonly">×</text>
			</view>
			</view>
		</scroll-view>
		</view>
...
<script>
	...
	export default {
		...
		props: {
			...
			upImgBasic: {type: Object,default() {return {}}},
		...
		computed: {
			...
			upImgConfig() {
				const basicImgConfig = {
					...
					// 上传文字描述(仅限四个字)
					upTextDesc: this.$i18nMsg('pageTip-uploadImg'),// 中文：上传图片
	...
	// #ifdef APP-PLUS
	upload_file_server = async (_this, configs, upload_ready_list, j) => {//将没有上传的图片批量上传
		let basicConfig = {
			url: configs.basicConfig.url || ""
		}
		...
		const upload_task = await uni.uploadFile({//开始上传
			...
			async fail(err) {
				uni.showLoading({
					title: _this.$i18nMsg('pageTip-pictureUploadFailed') //图片上传失败!
				});
	...
	// #ifdef H5
	const up_img = function (_this, configs, file, index, upload_picture_list_length) {
		let basicConfig = {
			url: configs.basicConfig.url || ""
		}
		let baseUrl = _this.baseUrl || _this.defaultUrl;
		return new Promise((resolve, reject) => {  
			...
			param.token = _this.token
			const upload_task = uni.uploadFile({//开始上传
				url: baseUrl + basicConfig.url,
				header: {
					'token':_this.token
				},
	...
	const uploads = async (_this, configs, upload_ready_list, upload_picture_list_length) => {
		const that=this;
		let arr = [];
		for(let k = 0,len = upload_ready_list.length; k < len; k++ ){
			arr[k] = await up_img(_this, configs, upload_ready_list[k], k, upload_picture_list_length).then((res) => {
				if (res.statusCode == 200) {
					let result = JSON.parse(res.data);
					if(result.success){
	...
	upload_file_server = async (_this, configs, upload_ready_list, j) => {//将没有上传的图片批量上传
		const upload_picture_list_length = _this.upload_picture_list.length
		const upload_tasks = uploads(_this, configs, upload_ready_list, upload_picture_list_length)
		upload_tasks.then(() => {
