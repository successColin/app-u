<template name="mQrcode">
	...
	<view v-if="element.visiabled">
		<view class="mQrcode formElemets" 
		 ...
		 element.readonly?'readonly':'']">
			...
			<view class="content" :class="{hasrelationTab:element.relationTabId}">
				<view class="qrcode-item">
					...
					<m-button v-if="value" class="button select" :isFontBtn="true" btnClass="mainColor">
						<block slot="buttonIcon" style="display: inline-block;">
							<m-font-icon icon="APP_59" @click="checkQrcode" :iconSize="38"></m-font-icon>
						</block>
					</m-button>
				</view>
			</view>
			...
			<view class="qrcode-pop" v-if="showCode">
				<view class="qrcode-wrap">
					<tki-qrcode class="qrcode-image" cid="myQR" ref="myQR"
						:val="value" 
						:onval="onval" 
						:loadMake="loadMake" 
						...
						:longtap="true"
						@result="qrR" />
					<p>{{$i18nMsg('basic-saveQrcode')}}</p>
					<button class="cancel-qrcode" type="default" @click="checkQrcode">取消</button>
				</view>
			</view>
		</view>
	</view>
