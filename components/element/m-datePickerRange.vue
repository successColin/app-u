	...
	<view class="dateRange">
		<view class="mDatePicker noBorder" :class="[element.labletPositon]">
			...
			<div class="content startDate">
				<input class="input" :value="value.start" disabled="true" @tap="showPicker('start')"/>
			</div>
		</view>
		<view class="mDatePicker" :class="[element.labletPositon]">
			<label class="text">{{ $i18nMsg("time-to") }}</label>
			<div class="content">
				<input class="input" :value="value.end" disabled="true" @tap="showPicker('end')"/>
			</div>
		</view>
	</view>
</template>
...
<script>	
	export default {
		props:{
			...
			value:{type:Object,default(){return {start:"",end:""}}},//值
		},
		methods:{
			showPicker(type){
