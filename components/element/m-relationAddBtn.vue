...
<template name="mRelationAddBtn">
	<view class="mButton" :class="[btnClass]" v-show="static_visiabled">
		<div :style="{fontSize: pxToREM(fontSize) }" @tap.stop="handleClick" style="z-index: 1000;"> 
...
<script>
	...
	export default {
		...
		props: {
			...
			btnColor:{
				validator(value){
					...
					const colorRGB = /^[rR][gG][Bb][Aa]?[\(]((2[0-4][0-9]|25[0-5]|[01]?[0-9][0-9]?),){2}(2[0-4][0-9]|25[0-5]|[01]?[0-9][0-9]?),?(0\.\d{1,2}|1|0)?[\)]{1}$/
					if(!value){
						return true
					}else{
		...
		methods: {
			...
			addRelation(relations){//添加数据
				if(relations.length==0 || !this.dataId) return
				formApi.batchInsertOrUpdate(this, {
