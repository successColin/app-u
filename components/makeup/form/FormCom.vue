<template name="mForm">
	<view class="mForm animated fadeIn">
		...
		<view v-if="element.visiabled" :class="{splitline:element.type === 'splitLine',
		 'form-items':element.type !== 'splitLine'&& element.baseType !==8 && element.baseType !==32 && element.baseType !==13 && showElementType.indexOf(element.baseType)!==-1}">
			...
			<!-- 二维码 -->
			<m-qrcode class="item" v-else-if="showElement(element.baseType,[50])" :sourceType="sourceType"
			:element = "element"
			:value="currentFormData | formValue(element)">
			</m-qrcode>
			...
			<!-- 图片上传-->
			<m-upload v-else-if="showElement(element.baseType,[8,13])" :sourceType="sourceType" 
			 :uploadNum="-1"
			 :element = "element"
			...
			<m-composite class="item" v-else-if="showElement(element.baseType,[47])" :element = "element"
...
export default {
	...
	props:{
		...
		tabStaticParams:{type:Object,default(){return{}}},//面板固定变量
		formConfig:{
			type:Object,
			default(){
				return {
		...
		showElementType:{
			type: Array,
			default: function () {
				return [1,2,3,8,9,10,12,13,15,17,20,21,22,25,36,32,39,46,45,47,50]
			}
		},
	},
	computed: {
		...mapState("form", ["filterSQLParams","currentBottomBtns","currentComboBoxs","filePassList"]),
		...
		selectValueArray(){
			const baseData = [{value:"",label:this.$i18nMsg('formTip-select')}] // 中文："请选择数据"
			let ary = []
	...
	methods:{
		...mapMutations("form",["SET_ALLBTNS","SET_FORM_OPTIONS","SET_FILEPASSLIST"]),
		...
		async elementValueChang(param){
			...
			if(triggerBaseType === 3 && param.value) {//赋值
				...
				if(key){
					...
					let fileParamsValue = fileParams[relationMainTab] ? fileParams[relationMainTab] : {}
				...
				if(param.element.defaultValue){
					for (let i = 0, len = param.value.length; i < len; i++) {
						try {
							if (param.value[i].id != "") {// 将未上传的图片信息保存到fileIds数组
		...
		if(param.dropDownList.length>0){
			formCom.getSelectValues(param,this).then( value => {
				for(let key in value){
					...
					static_down.unshift({value:'',label:this.$i18nMsg('formTip-select')})
