...
<script>
	...
	export default{
		...
		computed: {
			...
			...mapState("common",["tabsElements","tabsFormDatas","submitTabsDatas","frameUId"]),
			...
			btnInfo() {
				return function(item){
					const noChecked = [4,5,6,24,43,44],//不需要校验必填的按钮
								{
									...
									elementId,jumpMenuList,jumpTabList, executeCondition, defaultConditions
								} = item 
					//判断是否需要校验表单规则
					...
					if(!static_elementFormat){//如果没有设置刷新类型
						...
						else if(this.tabInfo.templateType === "list" && baseType === 6) static_elementFormat = 1 //列表上的删除按钮默认刷新本面板
		...
		methods:{
			...
			...mapMutations("common",["SET_TEMPLATE_PARAMS","SET_FORMREFRESH_PARENT","SET_COM_PAGERELOAD"]),
			...
			getJumpBundleParam(passParam = ''){
				...
				const jumpConfig = {
					passParam:passParam,mobileTabId,
			...
			checkExecuteCondition(formdata,text,type = 'form',elements){
				...
				let condition = '',result,conditions=[],title=obj.isFalse,newTitle=''
				// const reg = /(?<=\{@@).*?(?=\})/g
				...
				if(type == 'list'){
					formdata.forEach(item=>{
						for(let i = 0;i<formula.length;i++){
							...
							let paramValue = elementId ? item[elementId].id : this.Static_GlobalFucs.Global_ParamFormat({key:itemName},this).id
							...
							if(variable){
								...
								const elementValue = elements.filter(obj=>obj.parameter == elementName)
								const valueId = elementValue.length ? elementValue[0].elementId : null
								value = valueId ? item[valueId].id : this.Static_GlobalFucs.Global_ParamFormat({key:elementName},this).id
							}
							value = value || null
						...
						if(!result && falseNames.length){
							...
							falseNames.forEach(name=>{
								...
								const elementId = element.length ? element[0].elementId : null
								const value = elementId ? item[elementId].name : this.Static_GlobalFucs.Global_ParamFormat({key:name},this).id || null
					...
					for(let i = 0;i<formula.length;i++){
						...
						const element = elements.filter(obj=>obj.parameter == formula[i].key)
						const itemName = element.length ? element[0].name : null
						const variable = JSON.stringify(formula[i].value).indexOf('@@')== -1
						const cond = formula[i].cond == '=' ? '===' : formula[i].cond
						let paramValue = itemName ? formdata[itemName] : this.Static_GlobalFucs.Global_ParamFormat({key:formula[i].key},this).id
						paramValue = paramValue || null
						let value = ''
						if(variable){
							value = JSON.stringify(formula[i].value).trim()
							...
							value = valueName ? formdata[valueName] : this.Static_GlobalFucs.Global_ParamFormat({key:elementName},this).id
						}
						value = value || null
						if(i == formula.length-1){
							if(cond == '===' || cond == '!='){
					...
					if(!result && falseNames.length){
						falseNames.forEach(name=>{
							...
							title = title.replace('{'+name+'}',labelName || this.Static_GlobalFucs.Global_ParamFormat({key:name},this).id || '')
						})
					}
					newTitle = title
				}
			...
			handleBtnClick(params){//按钮点击
				...
				//针对表单或者列表需要做单独的处理
				if(this.tabInfo.templateType === "list"){//列表
					const listCheckIds = this.tabStaticParams.GlobalIds
					if(listCheckIds && listCheckIds.id) params.submitFormData.ids = listCheckIds.id//列表选中的值
					if(params.executeCondition && listCheckIds){
						...
						const isExecuteCondition = this.checkExecuteCondition(listCheckDatas,params.executeCondition,'list',elements)
						if(!isExecuteCondition) return
					}
					if(params.defaultConditions == 1){
						if(!listCheckIds || !listCheckIds.id || listCheckIds.id.indexOf(',') != -1){
							uni.showModal({
							    title: '',
							    content: this.$i18nMsg("lang-select-one"),
								showCancel: false
							});
							return
						}
				...
				//按钮前置触发条件判断
					const isExecuteCondition =  params.executeCondition && this.checkExecuteCondition(this.submitTabsDatas[tabId],params.executeCondition,'form',this.tabInfo.mobileTabs[this.tabInfo.mobileTabId].elements)
					if(params.executeCondition && !isExecuteCondition) return
					const {relationMainTab} = this.tabInfo
					if(params.type === 'saveBtn' && this.tabInfo.formType === "batch" && relationMainTab){ //表单上的批量处理，如果是保存按钮并且是需要批量处理
				...
				if(this.tabInfo.templateType === "list" && params.type === 'delBtn' && !params.submitFormData.ids){
					 this.Static_GlobalFucs.showToast({title:this.$i18nMsg('formTip-select')}) // 中文：请选择数据
					 return 
				}
			...
			isEligible(preconditions){//是否符合条件
				if(!preconditions) return true //如果没有匹配条件默认符合
					...
					const paramFormatConfig = {
						otherTabId:relationMainTab,mobileTabId,bundleParams,
						tabsElements:this.tabsElements,
						tabsFormDatas:this.tabsFormDatas,
						tabStaticParams:this.tabStaticParams
					}
