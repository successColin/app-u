import {TRIGGRT_NOTWEB, TRIGGRT_BaseValue} from '@/common/functions/trigger.js';
...
class BtnsComs {
	...
	triggerClassify(params,_this){
		...
		let triggersNoWeb = TRIGGRT_NOTWEB(baseconfig),
			...
			paramValueArray = [...triggerIds.paramValues,...pTriggerIds.paramValues,...hTriggerIds.paramValues]
		...
		for(let i= 0,len=paramValueArray.length;i<len;i++){
			...
			let paramValues = _this.Static_GlobalFucs.Global_ParamFormat(config,_this)
			//判断是否值为默认值
			let basevalue = {}
			if(paramValues.element) basevalue = TRIGGRT_BaseValue(paramValues.element.baseType,1,paramValues.id)
	...
	normalTriggers(params,_this){
		if (params.static_triggerIds || params.static_procedure_triggerids) { //后端触发器和存储过程
	...
	normalEvents(normalEventsParams,_this){
		...
		Promise.all([this.normalTriggers(normalTriggersParams,_this), this.httpEvent(httpParams,_this)]).then((result) => {
			let isFail = result.find(item => !item || item.success === false) || ""
			if(isFail) {
				throw isFail
				return 
			}
	...
	saveEvents(saveParams,_this){
		...
		formData.static_procedure_triggerids = procedure_triggerIds //存储过程触发器id
		formData.static_globalMap = static_globalMap //全局变量、面板变量
		if(formData.static_elementId){//如果有static_elementId说明是多条新增
	...
	archiveEvents(saveParams,_this){
		...
		if("@@null" in formData.static_globalMap) delete formData.static_globalMap["@@null"]
	...
	getJumpBundleParam(jumpConfig = {}, _this){//获取跳转菜单变量
		//获取跳转菜单的参数
		const passParam = jumpConfig.passParam
		let passParamObj = ""
		if(passParam) try {passParamObj = JSON.parse(passParam)}catch{}
		if(passParamObj){
			let basicConfig = {
			...
			for(let i = 0,len=passParamObj.length;i<len;i++){
				...
				let relValue = _this.Static_GlobalFucs.Global_ParamFormat(basicConfig,_this)
				relValue.relationColumnType = passParamInfo.type || relValue.relationColumnType
				if(!passParamValue){
					passParamValue = {}
	...
	click(params,_this){
		...
		if (elementId) {
			...
			triggerValue = this.triggerClassify({
				elementId:elementId,
		...
		if (normalEvent.indexOf(params.type)!==-1) { //如果为普通按钮，直接跳转面板
			...
			if(isGoOther === 'bundle'){
				...
				if(jumpBundleInfo){
					...
					common_inParam_selectByRelationId({bundleId:jumpBundleId},_this).then( value => {
						const jumpTagParam = params.jumpBundleParam
						let jumpInParams = {}
						value.map( item => {//处理输入参数
				...
				if(templateType==='list' && tabStaticParams && tabStaticParams.GlobalIds){//将列表上面板固定变量GlobalIds也带过去
				...
				}else if(normalBtns.indexOf(params.type) !== -1 && templateType==='list'){
					//如果是普通按钮并且是列表上的普通按钮是，设置type为batch
					//“batch”在表单展示类型时就需要进行批量处理
					tabParams.params.type = "batch"
					
				}
				if(mobileTabsBYRelation.mobileTabShowType === 6) { tabParams.params.ids = params.submitFormData.ids }//如果为批量面板
				_this.SET_TEMPLATE_PARAMS(tabParams) //设置下一个面板的参数
				_this.Static_GlobalFucs.gotoOtherRoute(tabParams,static_mobileTabShowType)//跳转其他路由
			}
			return "normalEvent"
		}
			...
			success: async function(res) {
				if (res.confirm) {
					...
					if(params.type === 'delBtn' ){
						if(params.submitFormData.ids){//批量删除
					...
					}else if (params.type === 'saveBtn') { //点击保存按钮
					...
					}else if (params.type === 'batchSaveBtn') { //点击批量保存按钮
						submitParams.formData = params.submitFormData
						submitParams.formData.static_tableName = params.relationTableName
						result = await _thisBtnCom.batchSaveEvents(submitParams,_this)
					}else if(params.type === 'archiveBtn' || params.type === 'normalArchiveBtn'){
