 ...
 * @Desc: 文件预览，包括图片、文档等
*/
const previewImage = (current,urls = []) => { //预览图片
	let config = {current}
	if(urls.length > 0) config.urls = urls
	else config.urls = [current]
	uni.previewImage(config);
}
const appDownloadFile = (file, _this) => {//下载文件
	const {id, name, fileLength, url} = file

	uni.downloadFile({//下载后文件会存在临时路径中，因为下次不想想再下载将文件存储到实际路径中，以便下次获取
	  url: url,
...
export const PREVIEW_FILE = (file, _this) => {//文件预览
	const {name, url} = file
	...
	console.log(type)
	if(picTypes.indexOf(type) !== -1){//如果是图片类型的直接预览
		previewImage(url)
	}else if(previewTypes.indexOf(type) !== -1){//如果是直接可以预览的文件，比如pdf与txt
		...
		//#endif
	}else if(type === 'mp3'){//如果是MP3
		previewMP3(url)
	}else if(type === 'mp4'){//如果是MP3
		video.type = "video"
		video.url = url
	}else {
		uni.showModal({
...
export const PREVIEW_DOWNLOAD_FILE = (file, _this) => {//文件下载
	const {url} = file
	if(!url) {
		uni.showModal({
			title: _this.$i18nMsg('basic-modalTip'), // 中文：'提示'
