...
//全局方法
const static_functionName = ["@@ParamKey","@@GetDuration","@@GetUserProperties","@@StringifyFormData","@@CompoundArithmetic"]
const systemFunction = {
	...
	GetUserProperties: (key) => {
		...
		if(functionKeyArry.length>1) param.relationColumnType = functionKeyArry[1]
		return param
	},
	//获取表单序列值
	StringifyFormData: (mobileTabElements=[],formData = {}) => {
		let count = 0,formDatas={}
		mobileTabElements.map(item => {
			const {baseType,name,relationColumnType} = item,
						formKey = `key${count+1}`
			if(NO_SUBMIT_ELEMENT_TYPE.indexOf(baseType) !== -1 || !name) return false //不提交的控件不不处理
	...
	GetDuration: (key,mobileTabElements=[],formData = {}) => {
		...
		if(isNaN(startTime) && isNaN(endTime) && !isNaN(Date.parse(startTime)) && !isNaN(Date.parse(endTime))){
			...
			else if(durationType === "@@StaticHours") total = Math.floor(durationTime/1000/60/60)
			else if(durationType === "@@StaticSeconds") total = Math.floor(durationTime/1000)
	...
	CompoundArithmetic: (key,mobileTabElements=[],formData = {}) => {
		...
		params.forEach(name=>{
			const parameterElement = mobileTabElements.find( item => '@@'+name === item.parameter) || ""
		...
		try{
			...
			uni.showToast({
				title: '四则运算除数不能为空或0',
				// title: _this.$i18nMsg('lang-compound-arithmetic'),
				duration: 1000,
				icon:'none'
			});
		}
	...
	if(!isParam(params.key)) return {id:params.key,name:params.key}
		...
		if(key1 in systemParams){//是否为全局变量
			...
			if(functionName == '@@ParamKey'){//如果有方法名。默认走方法
				...
				const mobileTabElements = tabsElements[mobileTabId] || [],//本面板控件
					  mobileTabData = tabsFormDatas[mobileTabId] || {}//本面板数据
				paramValueFormat = systemFunction.StringifyFormData(mobileTabElements,mobileTabData);
			}else if(functionName == '@@CompoundArithmetic'){
				const mobileTabElements = tabsElements[mobileTabId] || [],//本面板控件
					  mobileTabData = tabsFormDatas[mobileTabId] || {}//本面板数据
				paramValueFormat = systemFunction.CompoundArithmetic(key,mobileTabElements,mobileTabData);
			}
		}else if(key1 in FrameVariable){//框架变量
			paramValueFormat = FrameVariable[key1]
		}else if(key in bundleParams){ //菜单变量
			paramValueFormat = bundleParams[key]
		}else if(key1 in tabStaticParams){//面板固定变量
