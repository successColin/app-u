...
//全局方法
const static_functionName = ["@@ParamKey","@@GetDuration","@@GetUserProperties","@@StringifyFormData"]
const systemFunction = {
	...
	StringifyFormData: (mobileTabElements=[],formData = {}) => {
		...
		if(count) formDatas.count = count
		return {
...
const Static_ParamFormat = (params,_this) => {
	...
	if(!isParam(params.key)) return {id:params.key,name:params.key}
		...
		if(key1 in systemParams){//是否为全局变量
			...
			console.log(JSON.stringify(mobileTabElements))
