<template>
	<view class="language">
		<m-back-bar>
			<block slot="barTitle">{{$i18nMsg('page-switchLanguage')}}</block><!--中文：我的二维码-->
		</m-back-bar>
		<view class="menu-container">
			<view class="user-bundle" @click="switchLanguage('zh_CN')">
				<view class="bundle-name" :class="language=='zh_CN'?'current':''">中文</view>
				<view class="font-icon" v-if="language=='zh_CN'">
					<m-font-icon icon="iconxuanzhong" :color="themeColor" :iconSize="iconSize"></m-font-icon>
				</view>
			</view>
			<view class="user-bundle" @click="switchLanguage('en_US')">
				<view class="bundle-name" :class="language=='en_US'?'current':''">English</view>
	...
	export default {
		...
		methods: {
			switchLanguage(lang) {
				...
				this.$http.post('user/switchLanguage/APP/'+lang)
				.then(res => {
					console.log(res)
					if(res.data.success){
						uni.redirectTo({
