<template>
	<view class="user-info userInfo">
		<m-back-bar>
			<block slot="barTitle">{{$i18nMsg('lang-userinfo-title')}}</block><!--中文：个人信息编辑-->
			<block slot="barRight">
				<m-button
					@elementClick="handleTopClick(btn.type,btn.relationTabId)">
		...
		<view class="user-info-list">
			<view class="user-info-item user-image-item">
				<!-- <view>{{$i18nMsg('lang-userinfo-header')}}</view> --><!--中文：头像-->
			...
			<view class="user-info-item">
				<view>{{$i18nMsg('user-PhoneNumber')}}</view>
				<input class="uni-input" :placeholder="$i18nMsg('user-PhoneNumber')" v-model="tel" value="" />
			</view>
			<view class="user-info-item">
				<view>{{$i18nMsg('user-Email')}}</view>
				<input class="uni-input" :placeholder="$i18nMsg('user-Email')" v-model="email" value="" />
			</view>
			<view class="user-info-item">
				<view>{{$i18nMsg('lang-userinfo-signature')}}</view><!--中文：个人签名-->
				<textarea class="uni-input" auto-height :placeholder="$i18nMsg('lang-userinfo-please-signature')" v-model="personalSignal" value="" /><!--中文：请输入个人签名-->
			</view>
		</view>
		<view class="save-item">
			<m-button :btnTilte="$i18nMsg('basic-save')" btnClass="mainColor" @elementClick="UpdateDate"></m-button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			UpdateDate(){//保存修改信息、头像
				...
				this.$http.post('user/update', submitparam)
				.then(res => {
					const result = res.data
					console.log("==========================================================")
					if(result.success){
						uni.showToast({
							title: this.$i18nMsg('lang-myself-ModifiedSuccess'),
							duration: 1000,
							icon:'none'
						});
						...
						uni.showToast({
							title: this.$i18nMsg('lang-userinfo-faile'),// 中文：'修改失败',
							duration: 1000,
							icon:'none'
						});
					}
