...
<!-- 			<view class="sunsin_picture_list">
			...
			<view class="save-item">
				<m-button :btnTilte="$i18nMsg('basic-save')" btnClass="mainColor" @elementClick="insertUpdateDate()" ></m-button>
			</view>
		</view>
	</view>
</template>
			...
			insertUpdateDate(){//新增编辑数据
				...
				uni.request({
					...
					success: (res) => {
						...
						if(res.data.success){
							uni.showToast({
								title: _this.$i18nMsg('pageTip-successfulPictureUpload'),
								duration: 1000
							});
							this.uploadPics = [];						
						}else{
							uni.showToast({
								title: _this.$i18nMsg('pageTip-pictureUploadFailed'),
								duration: 1000
							});
						}
					}
