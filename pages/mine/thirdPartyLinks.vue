<template name="thirdPartyLinks">
	<view style="width: 100%;">
		<m-back-bar>
			<block slot="barTitle">{{$i18nMsg('page-otherweb')}}</block><!--中文：我的二维码-->
		</m-back-bar>
		<view class="otherwebContent" v-if="outLinkList && outLinkList.length>0">
			<view class="item" v-for="(item,index) in outLinkList" :key='item.id' @click="openUrl(item.url,item.name)">
				<view class="title">{{item.name}}</view>
				<view class="webLink">{{ item.url + (isUrl(item.url) ? '': $i18nMsg('pageTip-notweb'))}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		created() {
			getHomePageThirdPartyLinks({},this).then( value => {
