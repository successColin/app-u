<template name="feedback">
	<view class="feedback">
		<m-back-bar>
			<block slot="barTitle">{{$i18nMsg('lang-myself-feedback')}}</block><!--中文:意见反馈-->
			<block slot="barRight">
				<m-button
		...
		<view class="banner-view">
			...
			<view class="tips">{{$i18nMsg('page-feedbackTips')}}</view>
			<view class="QA-title"><view><m-font-icon icon="iconchangjianwenti" color="#fff" :iconSize="30"></m-font-icon></view><span>{{$i18nMsg('page-feedbackQA')}}</span></view>
		</view>
		<view class="tags-container">
			<view class="user-bundle">
				<view class="bundle-name">{{$i18nMsg('lang-feedback-type')}}</view><!--中文：反馈类别-->
			...
			<view class="user-bundle">
				<view class="bundle-name">{{$i18nMsg('lang-feedback-select-menu')}}</view><!--中文：选择菜单-->
			...
			<view class="tags">
				<view class="tags-title">{{$i18nMsg('lang-feedback-feedback')}}</view><!--中文：反馈内容-->
				<!--中文：请填写反馈内容，您的意见是我们提升产品服务的阶梯！-->
				<textarea class="feedback-text" :placeholder="$i18nMsg('lang-feedback-feedback-placeholder')" placeholder-style="color:#D9D9D9;font-size:14upx;" v-model="description" style="height:200upx"/>
			</view>
		</view>
		...
		<view class="save-item">
			<m-button :btnTilte="$i18nMsg('basic-save')" @elementClick="saveFeedback" btnClass="mainColor"></m-button>
		</view>
		<!-- 图片上传-->
		<!-- <m-upload  
		...
		<view class="firstMenuPop" v-if="firstMenuShow">
			<view class="title-bar">
				<view class="barTitle">{{$i18nMsg('lang-feedback-select-menu')}}</view><!--选择菜单-->
			...
			<view class="menu-container">
				<view class="user-bundle" v-for="(item,index) in pickerMenuArray" @click="openSecondMenu(index)" :key="index">
		...
		<view class="firstMenuPop" v-if="secondMenuShow">
			...
			<view class="menu-container">
				<view class="user-bundle" v-for="(item,index) in secondMenuArray" @click="selectSecondMenu(item)" :key="index">
					<view class="bundle-name">{{item.name}}</view>
					<view class="font-icon" v-if="item.id==selectMenu.id">
						<m-font-icon icon="iconxuanzhong" color="#d81e06" :iconSize="iconSize"></m-font-icon>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				pickerValueArray:[
					{
						label:this.$i18nMsg('lang-feedback-system-defects'),// 中文：'系统缺陷',
						value: 1
					},
					{
						label:this.$i18nMsg('lang-feedback-functional-requirements'),// 中文：'功能需求',
						value: 2
					},
					{
						label:this.$i18nMsg('lang-feedback-optimization-suggestions'),// 中文：'优化建议',
						value: 3
					}
				],
				pickerMenuText:'',
		...
		methods: {
			...
			getPcFunction(type){
				...
				this.$http.post(url, {userId:state.userId})
				.then(res => {
			...
			saveFeedback(){
				this.$http.post('feedback/insert', {basetype:this.basetype,description:this.description,functionId:this.selectMenu.id,terminalType:this.terminalType,userId:state.userId,})
				.then(res => {
					const result = res.data
					if(result.success){
						uni.showToast({
							title: this.$i18nMsg('lang-feedback-feedback-success'),//中文：'意见反馈成功',
							duration: 1000,
							icon:'none'
						});
					}
...
<style scoped>
	...
	.firstMenuPop{padding-top: var(--status-bar-height)}
</style>
