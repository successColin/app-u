<template name="news">
					...
					<view class="list-search">
						<view class="search-wrap">
							<view class="search-view">
								<input v-model="searchListInfo.searchKey" @confirm="searchFiles()" name="keywords" type="text" confirm-type="search" class="keywords" :placeholder="$i18nMsg('formTip-placeholder')" placeholder-class="placeholderSearch" /> <!-- 中文：请输入关键字搜索 -->
							</view>
	...
	<!-- 	<view class="header">
			<view class="templateTitle">{{$i18nMsg('page-infoShow')}}</view>
			<view class="list-search">
				<view class="search-wrap">
					<view class="search-view">
						<input v-model="searchListInfo.searchKey" @confirm="searchFiles()" name="keywords" type="text" confirm-type="search" class="keywords" :placeholder="$i18nMsg('formTip-placeholder')" placeholder-class="placeholderSearch" /> 
					</view>
					<m-font-icon  icon="iconsousuo" :iconSize="30" class="sousuo"></m-font-icon>
				</view>
			</view>
			...
			<view class="" >
				<scroll-view class="scroll-view_H" scroll-x="true" >
					  ...
					  @click="changeTab(item.id,item.name)" v-for="(item,index) in tabList"
					  :key="item.id"
					  :style="[tabIndex == item.id && !showSearch?{'color':themeColorCode}:'']"
					  >
				   		<span class="scroll-view-item">{{item.name}}</span>
						<hr
						 class="tab_item_hr"
						 :style="[tabIndex == item.id && !showSearch?{'background-color':themeColorCode,'border-color':themeColorCode,'opacity':'1'}:'']"
						 />
				   	</view>
				</scroll-view>
			</view>
		...
		<view class="" v-for="(item,index) in tabList" :key="item.id" v-if="tabIndex === item.id">
			
			<view class="news_container" v-show="contentList.length > 0">
				<view v-for="item in contentList" :key="item.id" >
					<navigator v-if="item.showType === 1" open-type="navigate" :url="`/pages/news/newsDetail?id=${item.id}&groupName=${groupName}`">
						<view class="news_wrap">
							<view class="news_title">{{item.name}} | {{item.remark}}</view>
							<view class="news_img">
								<img v-if="item.picturePath1" :src="item.picturePath1" alt="" class="">
								<img v-if="item.picturePath2" :src="item.picturePath2" alt="" class="mgl">
								<img v-if="item.picturePath3" :src="item.picturePath3" alt="" class="mgl">
							</view>
							<view class="news_tips">{{item.pubDate}}　　{{item.comments}}  评论
							</view>
						</view>
					</navigator>
					<navigator   v-if="item.showType === 2" open-type="navigate" class="news_item" :url="`/pages/news/newsDetail?id=${item.id}&groupName=${groupName}`">
						<view class="news_wrap news_layout2">
							<view class="news_layout2_left" >
								<view class="news_title">{{item.name}} | {{item.remark}}</view>
								
								<view class="news_tips2">{{item.pubDate}}　　{{item.comments}}  评论
								</view>
							</view>
		...
		<view>
			<view class="showNoData" v-show="showMore === false && waitShow">没有更多信息了</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			searchList(){
				let _me = this;
				_me.$http.post('content/searchContentList',_me.searchListInfo)
				.then(res => {
			...
			getContentList(){
				let _me = this;
				_me.$http.post('content/getContentListByContentGroupId',_me.pageInfo)
				.then(res => {
			...
			getTabList(){
				let _me = this;
				_me.$http.post('contentGroup/getContentGroupListByOrgId' )
				.then(res => {
					console.log(res);
					if(res.data.success){
