<template>
	<view class="container">
		...
		<view class="canvasView" v-if="echartData.series.length>0">
			<echart class="ec-canvas" :canvasId="canvasId+'_'" :echartData="echartData" ref="templateChart" />
		</view>
		<!-- <view class="ectart-title">{{$i18nMsg('lang-echart-repair-proportion')}}</view> 中文：工单占比 -->
		<view class="canvasView" v-for="(item,index) in pieData">
			<echart class="ec-canvas" :canvasId="item.canvasId+'_'" :echartData="item" ref="templateChart" />
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			initData(){
				this.$http.post('currencyPcList/currencyGetDataByElementId',{elementId: this.chartData.elementId})
				.then(res => {
			...
			initPieData(data, pieData, xdata){
				let chartType, axisUnitY, ydata, chartName, yAxisIndex, pie = {
					series: [],
					xAxis: [
