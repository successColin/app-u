<template name="BRATemplate">
	...
	<view class="BRATemplate">
		<m-back-bar>  
			...
			<block slot="barRight">
				<top-btns-coms :tabInfo="tabInfo" :btnBasicInfo="btnBasicInfo" :tabStaticParams="tabStaticParams" @showComBox="showComBox"></top-btns-coms></top-btns-coms> 
			</block>
		</m-back-bar>
		...
		<ListCom ref="List" v-if="isShow" :tabInfo="tabInfo" :listConfig = "listConfig" 
			@clickList="clickList"
			@checkNode = "checkNode"
			@unCheckNode = "unCheckNode"></ListCom>
...
<script>
	...
	export default{
		components: {topBtnsComs,bottomBtnsComs,mInputSearch,ListCom,dropdownMenu},
		data (){
			return {
				//列表属性
				listConfig:{},
		...
		computed: {
			...
			...mapState("common", ["templateParams","tabsElements","tabsFormDatas","pageRealod"]), //面板参数
		},
		watch: {
		...
		methods: {
			...
			checkNode(param){//列表选中
				...
				this.tabStaticParams.GlobalElements = this.tabInfo.mobileTabs[this.tabInfo.mobileTabId].elements
			},
			unCheckNode(param){//列表取消选中
			...
			handelSearch(searchParam){//处理搜索
				...
				seniorFilterMap = {...this.listConfig.otherParams.seniorFilterMap,...seniorFilterMap}
		...
		onLoad(options){
			...
			if(!this.BusinessID){
				this.Static_GlobalFucs.showModal(this,{
					...
					content:this.$i18nMsg("pageTip-noBbusinessKey"),
					showCancel:false
				})
				return
			}
