...
<!-- 					<view class="search-wrap">
					...
					<view class="search-wrap">
						<view class="search-view">
							<input v-model="keywords" name="keywords" class="keywords" :placeholder="$i18nMsg('formTip-placeholder')" /><!--中文：请输入关键字搜索-->
						</view>
		...
		<view class="filepath">
			<view class="path-wrap">
				<p v-for="(item,index) in filePath" :key="item.createDate" @click="goToStep(item,index)"><m-font-icon icon="iconzhishikuweizhihuidaodiyiji" color="#C62127" :iconSize="iconSize" v-if="index == 0"></m-font-icon>{{item.name}}<span v-if="index<filePath.length-1">></span></p>
			</view>
		</view>
		<!-- 内部文档 -->
		<scroll-view scroll-y class="list-wrap" v-if="tabIndex == 1">
			<view v-for="(item,index) in fileData" :key="index" @click="getFiles(item,item.id)">
				<view class="fileItem">
			...
			<view v-for="(item,index) in getFileData" :key="index">
				<view class="fileItem">
					...
					<view class="downloadIcon">
						<a target="_blank" 
						 :href="`${baseUrl || defaultUrl}file/download?id=${item.id}&token=${token}
							&relationTableName=${relationTableName}
							&relationColumnName=${relationColumnName || 'id'}
							&relationId=${relationId}`" 
						 :download="`${baseUrl || defaultUrl}file/download?id=${item.id}&token=${token}
							&relationTableName=${relationTableName}
							&relationColumnName=${relationColumnName || 'id'}
							&relationId=${relationId}`">
							<m-font-icon  icon="APP_44" :iconSize="30" color="#9b9b9b" class="xiazai"></m-font-icon>
						</a>
					</view>
				</view>
			</view>
		...
		<view class="downloadPro" v-if="isLoading">
			<view class="downloadBox">
				<p class="downname i18n">{{$i18nMsg('basic-download')}} <span>{{downFileName}}</span> ...</p><!--中文：下载-->
	...
	export default {
		...
		data(){
			return {
				...
				tabIndex:1,
				tabList:[{
					name:this.$i18nMsg('lang-relationFile-associated'),value:1},// 中文：'关联文档'
				],
				filePath:[{name:this.$i18nMsg('lang-relationFile-associated'),id:1,typeId:2}],// 中文：'关联文档'
				tempFilePath:'',
				downFileName:'',
				downFileProsess:'',
				isLoading:false,
				...
				rootId: '',
				barTitle: this.$i18nMsg('lang-relationFile-associated'),// 中文：'关联文档'
				isPreviewVideo:'',
				isPreviewSound:'',
		...
		onBackPress() {  
			...
			if(filePath.length>1) {  
			  ...
			  this.filePath = arr;
			  if(!e || e.name == this.$i18nMsg('lang-relationFile-associated')){// 中文：'关联文档'
			  	this.getFiles();
			  }else{
		...
		computed:{
			...mapState(["bundleName","bundleId","BusinessID","token","baseUrl","defaultUrl"]),
		...
		methods:{ 
			setPageInfo(mobileTabsBYRelation, tabInfo = {}) {
				...
				const {fileRelationTableName,fileRelationColumnName, tableGroupName} = mobileTabsBYRelation
			...
			searchFiles(){
				...
				let fileParam = {relationId: _this.relationId,relationTableName: _this.relationTableName, name: _this.keywords,rootId: _this.rootId,filesRelationsId: item ? item.filesRelationsId : ''}
				if(this.relationColumnName) fileParam.relationColumnName = this.relationColumnName
			...
			getFiles(item,rootId,arr){
				...
				let fileParam = {relationId: this.relationId,relationTableName: this.relationTableName,rootId: rootId ? rootId : '',filesRelationsId: rootId ? item.filesRelationsId : ''}
				//如果有关联字段，需要在请求带上参数
				if(this.relationColumnName) fileParam.relationColumnName = this.relationColumnName
				if(this.tableGroupName) fileParam.tableGroupName = this.tableGroupName
				let api = 'file/getRelationFiles'
				...
				.then(res => {
					...
					if(rootId&&!arr){
						...
						this.filePath = [{name:this.$i18nMsg('lang-relationFile-associated'),id:1}];// 中文：'关联文档'
					}
			...
			downloadFile(id, name, fileLength) {
				...
				plus.io.resolveLocalFileSystemURL(FileSystemURL, function(entry) {
					...
					uni.showModal({
						...
						content: _this.$i18nMsg('choiceTip-whetherOpen') + name + _this.$i18nMsg('page-attachmentView')+'?',// 中文：是否打开附件
			...
			dowload(id, name, fileLength) {
				...
				const downUrl = `${this.baseUrl || this.defaultUrl}file/download?id=${id}&token=${this.token}`
				...
				dtask.addEventListener("statechanged", function(task, status) {
					...
					switch(task.state) {
						...
						case 4:
							...
							console.log(e);
											plus.nativeUI.alert(_this.$i18nMsg('pageTip-unableOpenFile')+"：" + e.message);//中文：无法打开此文件
										});
									}
								}
							});
