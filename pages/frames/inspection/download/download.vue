<template>
	<view class="inspection-download">
		<m-back-bar>
			<block slot="barTitle">{{$i18nMsg("basic-download")}}</block>
		</m-back-bar>
		
		<view class="progress-box-parent">
			<view class="progress-bg">
				<view class="progress-box">
					<text class="progress-title">{{$i18nMsg('inspection-routeTask')}}</text><!--巡检路线任务-->
					<progress
						class="progress"
				...
				<view class="progress-box">
					<text class="progress-title">{{$i18nMsg('inspection-pointTast')}}</text><!--巡检点任务-->
					<progress
						class="progress"
		...
		<view class="formTemplate-btns foot">
			...
			btnClass="mainColor" 
			:disabled="disabled"
			:btnTilte='$i18nMsg("basic-download")'
...
<script>
	...
	import { readyDB, insertIntoInspectionDo, insertInspectionpointDo, selectpoint, selectInspection, dropInspectionTables } from '@/static/js/DB.js'
		...
		methods: {
			...
			downloadOver(){
				...
				uni.showModal({
					...
					content:this.$i18nMsg("pageTip-downloadSuccessful"),//中文:'下载成功',
					showCancel:false,
			...
			handleBttomBtnClick () {
				...
				uni.showModal({
					...
					content:that.$i18nMsg("inspection-isDownload"),// 中文：'下载会清空原本所有数据，是否下载？',
					confirmText: that.$i18nMsg("basic-confirm"),
					cancelText: that.$i18nMsg("basic-cancel"),
					success:function(res){
						if(res.confirm){
							uni.showLoading({
								title:that.$i18nMsg('pageTip-readyDownload')//中文；'准备下载'
							})
							dropInspectionTables(function (res) {
								readyDB(function () {
									uni.showLoading({
										title:that.$i18nMsg('pageTip-downloading')//中文；'下载中...'
									})
			...
			downIntoInspection (pageNum) {
				let that = this;
				inspectionApi.getInspectionDo(that, {
					pageNum:pageNum,
					pageSize:100
				},that.userId)
				.then(res => {
					if(res.list.length === 0) return;
					insertIntoInspectionDo(res.list, function () {
						let totalPageNum = Math.ceil(res.total / 100);
						let nextPageNum = pageNum + 1;
						that.inspectionRoute = ((pageNum / totalPageNum).toFixed(1) * 100);
						if(nextPageNum <= totalPageNum && that.isDownload) that.downIntoInspection(nextPageNum);
					}, function (e) {
						uni.hideLoading();
						uni.showModal({
							title:that.$i18nMsg("basic-modalTip"),//中文：'提示',
			...
			downIntoInspectionPoint (pageNum) {
				let that = this;
				inspectionApi.getInspectionPotinDo(that, {
					pageNum:pageNum,
					pageSize:100
				},that.userId).then(res => {
					if(res.list.length === 0){
						uni.showModal({
							title:that.$i18nMsg("basic-modalTip"),//中文：'提示',
							content:that.$i18nMsg('inspection-noTask'),//中文：'没有点检任务',
							confirmText: that.$i18nMsg("basic-confirm"),
							showCancel:false
						})
						uni.hideLoading();
					...
					insertInspectionpointDo(res.list, function () {
						let totalPageNum = Math.ceil(res.total / 100);
						let nextPageNum = pageNum + 1;
						that.inspectionPoint = ((pageNum / totalPageNum).toFixed(1) * 100);
						if(nextPageNum <= totalPageNum && that.isDownload) that.downIntoInspectionPoint(nextPageNum);
					}, function (e) {
						uni.hideLoading();
						uni.showModal({
							title:that.$i18nMsg("basic-modalTip"),//中文：'提示',
