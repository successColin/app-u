	...
	<view class="routeFilter">
		...
		<view class="item">
			<view class="mLabel formElemets left">
				<label class="text" :class="name?'':'drakcolor'">{{$i18nMsg('inspection-routeName')}}:</label><!--中文：路线名称-->
				<view class="content">
		...
		<m-datePicker class="item" :element = "startdateElement" :value="startdate" @select="selectData('startdate')">
		</m-datePicker>
		<m-datePicker class="item" :element = "enddateElement" :value="enddate" @select="selectData('enddate')">
		</m-datePicker>
		<!-- 底部按钮 -->
	...
	export default {
		data() {
			return {
				...
				startdate:'',
				enddate:'',
				startdateElement:{visiabled:1,labletPositon:"left",label:this.$i18nMsg('page-startTime')+":",name:"startdate"},//中文："开始时间
				enddateElement:{visiabled:1,labletPositon:"left",label:this.$i18nMsg('page-endTime')+":",name:"enddate"}//中文："结束时间
			}
		},
		components:{
			mDatePicker,
		...
		methods: {
				...
				if (this.type === 'startdate' || this.enddate){
					...
					if(startdateTime > enddateTime){
						uni.showModal({
							...
							content:this.$i18nMsg('check-startAndEnd'),// 中文:'开始日期不能大于结束日期'
					...
					const startdateTime = new Date(this.changeDate(this.startdate)).getTime();
					const enddateTime = new Date(this.changeDate(e.label)).getTime();
					if(startdateTime > enddateTime){
						uni.showModal({
							title:this.$i18nMsg('basic-modalTip'),//中文："提示",
							confirmText: this.$i18nMsg("basic-confirm"),
							cancelText: this.$i18nMsg("basic-cancel"),// 中文:'结束时间不能小于开始时间'
							content:this.$i18nMsg('check-endAndStart'),
						})
					}else{
						this[this.type] = e.label;
					}
			...
			handleBttomBtnClick(){
				...
				prevPage.$vm.params = { startDate:this.startdate, endDate:this.enddate, name:this.name };
				uni.navigateBack();
			},
			changeDate(date){
