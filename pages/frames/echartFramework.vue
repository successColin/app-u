<template name="echartFramework">
	...
	<view class="echartFramework"> 
		<m-back-bar :class="{transparent:showDefault}" :colorCode="colorCode" :statusBarBgColor="statusBarBgColor">  
		...
		<view class="echart-content">
			<view class="statistics-list">
				<view class="statistics-item" v-for="(item,index) in totalData" :key="index" @click="handleNav(item)">
					<view>
						<m-font-icon :icon="item.iconName" :color="item.colorCode" :iconSize="iconSize"></m-font-icon>
			...
			<view class="echart-list">
				<view v-for="(item,index) in chartData" :key="index">
					<echarts-template :chartData="item" :canvasId="item.elementId" ref="templateChart" ></echarts-template>
				</view>
			</view>
		</view>
	</view>
  ...
  import echartsTemplate from '@/pages/common/echartsTemplate.vue';
	...
	export default{
		...
		computed: {
			...
			...mapState("home", {newBundle:'newBundle',workSpaceList:'workSpaceList',}),
			height_() {
		...
		methods: {
			...
			getTotalData(elementId){
				...
				this.$http.post('currencyPcList/currencyGetTotalByElementId', {elementIds:elementId.elementId,chartMap:chartMap},
				...
				for(let n in TotalData){
					for(let i=0;i<this.gatherData.length;i++){
						if(n == this.gatherData[i].elementId){
							...
							this.totalData = this.totalData.sort(compare('elementId'))
						}
					}
				}
			},
			...
			handleNav(item){//如果没有框架信息，提示功能正在开发中
					...
					if(!isJump){
						_this.Static_GlobalFucs.showModal(_this,{
							title:_this.$i18nMsg("pageTip-noPromiss"),
							showCancel:false
						})
						return 
					}
					if(!bundle){
						_this.Static_GlobalFucs.showModal(_this,{
							title:_this.$i18nMsg("lang-no-relation"),
							showCancel:false
						})
						return 
					}
					...
					const url=`/pages/${template}?title=${bundle.name}&bundleId=${bundle.id}&mainTab=1`
					uni.navigateTo({
						url:url
					})
			}
