		...
		<div class="content register animated fadeIn"> 
			<view class="register-header">
				<div class="registerheader-item">
					<span>{{$i18nMsg('lang-register')}}</span><!--中文;快速注册-->
				</div>
				<div class="registerheader-text">{{$i18nMsg('lang-register-convenient-quick')}}</div><!--中文：免验证，方便快捷-->
			</view>
			<div class="register-content">
				<view class="register-item">
					<!-- 中文：手机号 -->
					<m-input-lable class="register-item" :label="i18nMsg('user-PhoneNumber')" name="telephone" 
				...
				<!-- 中文：姓名 -->
				<view class="register-item">
					<m-input-lable :label="$i18nMsg('user-name')" name="name"
			...
			<view class="register-foot">
				<!-- 中文：立即注册 -->
				<m-button ref="registerBtn" :btnTilte="$i18nMsg('lang-register-now')" btnClass="mainColor" @elementClick="register"></m-button>
			</view>
		</div> 
	</div>
</template>
		...
		methods:{
			...
			register () {
				...
				const checkRes = this.Static_GlobalFucs.checkForm({
					rule:[
						{name:"telephone", checkType : "notnull", checkRule:"",  errorMsg:_t.$i18nMsg('user-PhoneNumber') + _t.$i18nMsg('check-notNull')},//中文："手机号不能为空"
						{name:"name", checkType : "notnull", checkRule:"",  errorMsg:_t.$i18nMsg('user-name') + _t.$i18nMsg('check-notNull')}//中文："姓名不能为空"
					],
					data:this.formData
				})
				if(!checkRes.isCheck){
