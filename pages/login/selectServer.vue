	...
	<view class="server-info">
		...
		<view class="serverContent">
			<view class="leftPart" :class="[themeClass_]">
				<ul>
					<li :class="tab=='server'?'selected':''" @click="changeTab('server')">{{$i18nMsg('lang-selectServe-server')}}</li><!--中文：服务器-->
					<li :class="tab=='language'?'selected':''" @click="changeTab('language')">{{$i18nMsg('lang-language')}}</li>
					<!-- <li>显示</li> -->
				</ul>
			</view>
			<view class="rightPart" v-if="tab=='server'">
				<view class="serverItem">
					<p>{{$i18nMsg('lang-selectServe-formal-environment')}}</p><!--中文：正式环境-->
					<view class="inputItem">
						<view class="inputBox">
							<input type="text" name="formalServer" v-model="formalServer" @blur="setServerUrl('formalServer')" />
						</view>
						<view class="radioItem">
							<radio value="1" @click="changeServer(1,'formalServer')" name="serverUrl" :checked="'1' == checkIndex" />
						</view>
					</view>
				</view>
				<view class="serverItem">
					<p>{{$i18nMsg('lang-selectServe-intranet-test-environment')}}</p><!--中文：内网测试环境-->
					<view class="inputItem">
						<view class="inputBox">
							<input type="text" name="inTestServer" v-model="inTestServer" @blur="setServerUrl('inTestServer')" />
						</view>
						<view class="radioItem">
							<radio value="2" @click="changeServer(2,'inTestServer')" name="serverUrl" :checked="'2' == checkIndex" />
						</view>
					</view>
				</view>
				<view class="serverItem">
					<p>{{$i18nMsg('lang-selectServe-extranet-test-environment')}}</p><!--中文：外网测试环境-->
					<view class="inputItem">
						<view class="inputBox">
							<input type="text" name="outTestServer" v-model="outTestServer" @blur="setServerUrl('outTestServer')" />
						</view>
						<view class="radioItem">
							<radio value="3" @click="changeServer(3,'outTestServer')" name="serverUrl" :checked="'3' == checkIndex" />
						</view>
					</view>
				</view>
				<view class="serverItem">
					<p>{{$i18nMsg("lang-selectServe-other")}}</p><!--中文：其它-->
					<view class="inputItem">
						<view class="inputBox">
							<input type="text" name="otherTestServer" v-model="otherTestServer" @blur="setServerUrl('otherTestServer')" />
						</view>
						<view class="radioItem">
							<radio value="4" @click="changeServer(4,'otherTestServer')" name="serverUrl" :checked="'4' == checkIndex" />
						</view>
					</view>
				</view>
			</view>
			...
			<view class="rightPart" v-if="tab=='language'">
				<view class="serverItem">
					<view class="inputItem">
						<view class="inputBox language">
							中文
						</view>
						<view class="radioItem language">
							<radio value="1" @click="changeLanguage('zh_CN')" name="language" :checked="'zh_CN' == language" />
						</view>
					</view>
					<view class="inputItem">
						<view class="inputBox language">
						...
						<view class="radioItem language">
							<radio value="2" @click="changeLanguage('en_US')" name="language" :checked="'en_US' == language" />
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		data() {
			return {
				barTitle:this.$i18nMsg("lang-selectServe-basic-configuration"),//中文：'基础配置',
				checkIndex:1,
				index:1,
				formalServer:'',
				inTestServer:'',
		...
		methods: {
			...mapMutations([ "UPDATE_SERVERURL_STATE", "UPDATE_THEME_COLOR","SET_LOGIN_CONFIG" ]),
			...
			setServerUrl(name){
				...
				switch(name) {
					...
					if(this.checkIndex == 1){url = this.formalServer};
					break;
					case 'inTestServer':
					uni.setStorageSync(name, this.inTestServer);
					if(this.checkIndex == 2){url = this.inTestServer};
					break;
					case 'outTestServer':
					uni.setStorageSync(name, this.outTestServer);
					if(this.checkIndex == 3){url = this.outTestServer};
					break;
					case 'otherTestServer':
					uni.setStorageSync(name, this.otherTestServer);
					if(this.checkIndex == 4){url = this.otherTestServer};
					break;
					default:
				...
				uni.getStorageInfo({
					success: function (res) {
						...
						for(let i=0;i<keys.length;i++){
							if(reg.test(keys[i]) || keys[i] == 'bundleVersions' || keys[i] == 'homeTemplate' || keys[i] == 'bundlePermissiones'){
