...
export const common_inParam_selectByRelationId = (param , _this) => {
	return _this.$http.post(selectByRelationId, param)
	.then(res => {
...
export const common_getDeviceCheck = (headId , _this) => {//获取设备盘点面板参数
...
export const common_inventoryScanInster = (param , _this) => {//设备盘点扫描
//扫描结果如果在盘点明显表中返回id进入盘点明显表
//扫描结果如果不在明显表了，新增一条盘点明显，并且状态为盘盈
	return _this.$http.post(inventoryScanInster, param)
	.then(res => {
		const result = res.data || {}
		return result && result.data
	})
