...
let commonApi = {
	getSearchSelectById(_this, params, isMultiple){//数据选择框通过id获取所有的信息
	...
	stateSelectData(_this, params){//查询状态列表
		return _this.$http.post(Path.stateSelectData, params,{
			noLoading:true
		})
		.then(res => {
			const result = res.data
	...
	currencyGetDropDown(_this, params){//通用下拉数据
		return _this.$http.post(Path.currencyGetDropDown, params,{
			noLoading:true
		})
		.then(res => {
			const result = res.data
