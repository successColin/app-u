...
import {TRIGGER_METHODTYPE, TRIGGER_Conditions} from '@/common/constant/triggers.js'
...
export const TRIGGRT_BaseValue = (elementType, type = 1, name) => {
	...
	let baseName = name
	if(dateArray.indexOf(elementType) !== -1 && name) baseName = dateFormat.formatDate(baseName, "yyyy-MM-dd")
	let base = ELEMNT_BASE_VALUE.find( item => item.elmentType.indexOf(elementType) !== -1) || {}
	let baseValue = {id:name}
	if(type === 1 && base.basicData === BasicDatasEnum.getCodeByName(baseName)) baseValue = {id:"",name:""}
	else if(type === 2 && !name) {
...
export const TRIGGRT_GetRule = (element, $i18nMsg = '') => {//设置校验规则
	let dataRule = CHECKED_TYPE[element.dataFormat] || "",//数据格式
		ruleArray = [],
		rule = ''
	if(element.visiabled){//只有可见的控件设置校验规则
		if(element.requisite){//必填
...
function getTriggerByEvent(element, triggerCur){
	let triggerEvent = {}
	triggerCur.map( item => {
		if(item.elementEvent in triggerEvent){//如果对应的事件已经存在
...
function compareTriggerMuch(config = {}, endTriggerNum){
	...
	try{endTriggerNumObj = JSON.parse(endTriggerNum)}catch(e){endTriggerNumObj = []}
	...
	for(let i = 0,len=endTriggerNumObj.length;i<len;i++){
		...
		endTriggerNum = Static_ParamFormat({...{key:endTriggerNum},...paramFormatConfig})
		//如果条件类型为AND
		if(methodType === 1){
		...
		}else if( methodType === 2){//如果条件类型为OR
			if(staticCondition(conditionType,endTriggerNum.id,key.id)){
				flag = true
				break
			}else{
...
function staticCondition(conditionType,endTriggerNum,value){
	...
	}else if(condition === 2){//小于
		return value < endTriggerNum
	}else if(condition === 3){//等于
		return value === endTriggerNum
	}else if(condition === 4){//大于等于
		return value >= endTriggerNum
	}else if(condition === 5){//小于等于
		return value <= endTriggerNum
	}else if(condition === 6){//不等于
		return value !== endTriggerNum
	}else if(condition === 7){//为空
		return value === false
	}else if(condition === 8){//不为空
		return value === true
	}
}
...
function compareTrigger(config = {},endTriggerNum,value){
	...
	if(conditiontype === 9){//为多控件
		return compareTriggerMuch(config, endTriggerNum)
	}else{
		const conditionType = TRIGGER_Conditions.find(item => item.value === conditiontype)
...
export const TRIGGRT_Events = (param) => {
	...
	for(let key in triggerEvent){
		...
		if(compareTrigger(compareTriggerConfig,endTriggerNum.id,param.value)){// 判断是否符合条件
	...
	let events = []
	if(type === 1) events = compareEvent //只去符合的事件
...
export const TRIGGRT_NOTWEB = (param) => { 
	const noWebTrigger = param.triggers.filter( item => item.triggerType!==1 && item.triggerType!==4) || []
	//获取对应的触发器id
	...
	for(let i = 0,len=noWebTrigger.length;i<len;i++){
		...
		if(trigger.triggerType === 2){//后端触发器
		...
		}else if(trigger.triggerType === 3){//存储过程
			...
			if(procedureMap) {
				for(let key in procedureMap){
					...
					if(procedureMap[key] && paramValues.indexOf(procedureMap[key]) === -1) 
						procedure_triggerIds.paramValues.push(procedureMap[key])
				}
			}
		}else if(trigger.triggerType === 5){//http
			...
			if(trigger.httpRequestList && trigger.httpRequestList.length>0) {
				...
				http_triggerIds.paramValues = [...http_triggerIds.paramValues,...result]
			}
		}
	}
