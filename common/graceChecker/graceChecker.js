...
module.exports = {
	...
	$i18nMsg:'',
	static_errorMsg:{},
	check : function (data, rule, $i18nMsg){
		this.$i18nMsg = $i18nMsg
		this.static_errorMsg = {//系统报错信息
			wrong : this.$i18nMsg ? this.$i18nMsg('check-wrong') : '填写有误',
			nan : this.$i18nMsg ? this.$i18nMsg('check-nan') : '非数字',
			greaterThan : this.$i18nMsg ? this.$i18nMsg('check-greaterThan') : '不能大于',
			lessThan : this.$i18nMsg ? this.$i18nMsg('check-lessThan') : '不能小于',
			notNull : this.$i18nMsg ? this.$i18nMsg('check-notNull') : '不能为空',
		}
		for(var i = 0; i < rule.length; i++){
			...
			for(let i = 0,len = checkTypeArray.length;i<len;i++){
				...
				if(notnull.indexOf(checkTypeArray[i]) === -1 && !params.value) continue
				flag = this.checkrules(params)
				if(!flag) break
			}
			if(rule[i].checkFunction){ //自定义方法
	...
	checkrules: function(params = {}){
		...
		switch (params.checkType){
			...
			case 'string':
				...
				if(!reg.test(params.value)) {this.error = params.errorMsg; flag = false;}
			break;
			case 'int':
			...
			break;
			case 'dateRange'://时间
				if(!params.checkRule){flag = false;return false} 
				...
				if(!minMax[0] && minMax[1]) {
					...
					if (valueDate < startDate || valueDate > endDate) {
						this.error = msg[0]+this.static_errorMsg.lessThan+msg[1]+"," + this.static_errorMsg.greaterThan +msg[2];
						flag = false
					}
				}
			break;
			case 'numberRange':
				...
				if(isNaN(valueData)) { this.error = this.static_errorMsg.nan; flag = false; }
				if(!params.checkRule){flag = false;return false}
				var minMax = params.checkRule.split(','),
					msg = params.errorMsg.split(',');
			...
			case 'betweenF': 
				var reg = /^-?[0-9][0-9]?.+[0-9]+$/;
				if (!reg.test(params.value)){this.error = params.errorMsg; flag = false; return false;}
				var minMax = params.checkRule.split(',');
				minMax[0] = Number(minMax[0]);
				minMax[1] = Number(minMax[1]);
				if (params.value > minMax[1] || params.value < minMax[0]) {
			...
			break;
			case 'same':
				if (params.value != params.checkRule) { this.error = params.errorMsg; flag = false;}
			break;
			case 'notsame':
				if (params.value == params.checkRule) { this.error = params.errorMsg; flag = false; }
			break;
			...
			case 'check.phoneno':
				...
				if (!reg.test(params.value)) { this.error = (params.label||'')+this.static_errorMsg.wrong; flag = false; }
			break;
			case 'zipcode':
				var reg = /^[0-9]{6}$/; 
				if (!reg.test(params.value)) { this.error = params.errorMsg; flag = false; }
			...
			case 'in':
				if(params.checkRule.indexOf(params.value) == -1){
					this.error = params.errorMsg; flag = false;
				}
			break;
			case 'notnull':
				let value = params.value + ""
				if(params.value == null || value.length < 1){
					this.error = params.label + this.static_errorMsg.notNull; flag = false;
				}
			break;
			case 'imgnotnull':
